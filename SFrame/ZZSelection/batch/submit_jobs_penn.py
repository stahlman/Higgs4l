...
def run_job_submission(**kwargs) :
    ...
    if submit :    
        ...
        if output.count('/xrootd') :
            make_output_dir_xrootd(output, starting_job_id, len(job_file_list), overwrite)
...
def SplitIntoJobs(in_path, kwargs ) :
    ...
    if Formatter is not None :
        ...
        for in_pathone in in_path:
            if in_pathone.startswith('DS'):
                arg_files += get_files_from_dq2(in_pathone, fkeys)
            elif in_pathone.startswith('/disk'):
	        arg_files += get_files_from_disk(in_pathone, fkeys)
	    else:
        ...
        for dq2line in dq2lines:
            if dq2line.find('root') != -1:
                if dq2line.find('srm:') != -1:
                    matched_files.append(dq2line.replace('srm://srm.hep.upenn.edu/disk/space00/','root://hn.at3f//'))
...
#---------------------------------------------------------
def get_files_from_disk(paths, key) :
    ...
    for each_path in paths :
       ...
       print 'searching /disk for %s' %each_path
	  
       if key is None: continue
       
       for k in key:
	  dirs = glob(each_path + '/*' + str(k) + '*')

	  for directory in dirs: 
	    for r,d,files in os.walk(directory):
        	for f in files:
		   if f.find('root') != -1:
                     matched_files.append(os.path.join(r,f))
...
def parse_xrd_dir(path, DEBUG=False) :
    ...
    # glob bitches
    #print '/xrootd'+path+'/*'
    for entry in glob('/xrootd'+path+'/*') + glob('/xrootd'+path+'/.*'):
...
#---------------------------------------------------------
def rmdir_xrootd(path) :
    
    #print "removing xrootd files with path",path    
    dirs, files, sizes = parse_xrd_dir(path)
    for file in files:
	print ('xrd hn.at3f rm '+path+'/'+file)
	os.system('xrd hn.at3f rm '+path+'/'+file)          
    for dir in dirs :
	rmdir_xrootd(path+'/'+dir)
    print ('xrd hn.at3f rmdir '+path)
    os.system('xrd hn.at3f rmdir '+path)
...
#-----------------------------------------------------------------------------------
#function to make all xrootd directories simultaneously because creating a directory is time consuming
def make_output_dir_xrootd(output, start_job_index, njobs, overwrite) :

     if (os.path.exists(output)):
	if (overwrite): 
	   rmdir_xrootd(output[len('/xrootd'):])
	else:
	   print 'Output directory already exists.  Add --overwrite to over-write the output directory  Exiting....'
	   sys.exit(0)
     #print "EXIT"
     #sys.exit(0)
     #print 'Creating xrootd directories'

     os.system('xrd hn.at3f mkdir %s' %(output[len('/xrootd'):]))
     #print ('xrd hn.at3f mkdir %s/Job_%04d' %(output[len('/xrootd'):], job_index))
     cmd = ''
     for i in range(njobs):
       cmd += ('xrd hn.at3f mkdir %s/Job_%04d  & ' %(output[len('/xrootd'):], start_job_index+i))	    
     os.system(cmd)
