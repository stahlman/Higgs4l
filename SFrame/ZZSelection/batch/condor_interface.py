...
parser.add_argument('--filelist-name',     dest='filelist_name',     default=None,        help='Use a text file that contains the input files')
...
parser.add_argument('--num-sim-transfers', dest='num_sim_transfers',  default=5,  type=int, help='Number of simultaneous transfers allowed')
parser.add_argument('--num-sim-host-transfers', dest='num_sim_host_transfers',  default=1,  type=int, help='Number of simultaneous transfers allowed per host')
...
def main() :
    ...
    # separator can be provided, default to new line
    if options.filelist_name is not None :
        filesfile = open(options.filelist_name, 'w')
        filesfile.write( options.filelist_sep.join( script_files ) )
        filesfile.close()
    ...
    if options.no_transfer_input :
	    ...
	    print 'Subscribed to queue %s' %options.file_queue_dir
            ...
            print 'My host is', socket.gethostname() 
            sys.stdout.flush()
    ...
    if options.input_command.count('_ATHENA_') :
        ...
        script_call = 'athena.py %s -c " InputFiles=[%s] %s " ' %(script_name, ','.join(['\'%s\''%f for f in script_files]), options.module_args)
    else :
        files_cmd = '%s %s' %(options.input_command, ','.join(script_files) )
        if options.filelist_name is not None :
            files_cmd = ''
        prog_call = '.'
        if script_name.count('.') :
            if script_name.split('.')[-1] == 'py' :
                prog_call = 'python'
        script_call = '%s %s %s %s' %(prog_call, script_name, files_cmd, options.module_args)
    ...
    if options.output is not None :
        ...
        for file in os.listdir('.') :
            ...
            if file[0] == '.' :
                continue
            ## -------------------------------------
            ## Instead of writing through hn.at3f write through at3f:1095
            ## to give faster transfers.  Keep old code for now
            ## ------------------------------------
            ...
            #sys.stderr.flush()
            print('xrdcp %s root://srm.at3f:1095/%s/' %(file, mod_output))
            sys.stdout.flush()
            sys.stderr.write('xrdcp %s root://srm.at3f:1095/%s/' %(file, mod_output))
            sys.stderr.flush()
            os.system('xrdcp %s root://srm.at3f:1095/%s/' %(file, mod_output))
            sys.stderr.write('done')
            sys.stderr.flush()
...
def enter_queue(queue_name, id) :
    ...
    while True :
        ...
        # otherwise, wait 10 seconds and check again
        with FileLock(queue_name, timeout=1000000, delay=5) :
            ...
            if len(lines) < options.num_sim_transfers:	        
		n_same_host = 0  #count number of other jobs on same host
		host = socket.gethostname()
		for line in lines:
		     if (line.split()[1] == host): n_same_host += 1
                if n_same_host < options.num_sim_host_transfers:
		  qfile.write(id + ' ' + host +'\n')
                  entered_queue = True
            qfile.close()
