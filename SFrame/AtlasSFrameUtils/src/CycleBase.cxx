...
:   m_entry_number(0),
    ...
    m_MC_channel_number( "channel_number", this ),
    m_branch_metadata(0),
    m_histoMap(0)
{
    ...
    DeclareProperty( "SFrame_dir",            c_sframe_dir= "." );
...
{
    CollectHists();
    //m_CollectToolHists(); //remove for now, causing unnecessary ERRORs
    ...
    m_logger << ALWAYS << "MC channel number: "
        << *m_MC_channel_number << SLogger::endmsg;

    TParameter<long> N_all_events("N_all_events", *m_all_events);
    TParameter<long> N_passed_events("N_passed_events", *m_passed_events);
    TParameter<long> MC_channel_number("MC_channel_number", *m_MC_channel_number);
    WriteObj( N_all_events );
    WriteObj( N_passed_events );
    WriteObj( MC_channel_number );
...
{
  if(directory.size()) return dynamic_cast<TH1F*> (m_hist( name.c_str(), directory.c_str()));
  else return dynamic_cast<TH1F*> (m_hist( name.c_str(), 0));
}


//____________________________________________________________________________
TH1F* CycleBase::Book( const std::string& name, const std::string& title, int nbins, float min, float max, 
        const std::string& directory/* = ""*/ )
{
  TH1F* result = 0;
  if(directory.size()) result = dynamic_cast<TH1F*> (m_hist( name.c_str(), directory.c_str()));
  else result = dynamic_cast<TH1F*> (m_hist( name.c_str(), 0));

  if(!result) {
    TH1F* histo = new TH1F(name.c_str(), title.c_str(), nbins, min, max);    
...
//     try
//     {
//         return dynamic_cast<TH1F*> (SCycleBase::Hist( name.c_str(), directory.c_str() ));
//     }
//     catch( const SError& error )
//     {
//         TH1F histo(name.c_str(), title.c_str(), nbins, min, max);
...
//____________________________________________________________________________
TH1F* CycleBase::Book( const std::string& name, const std::string& title, int nbins, const float* xbins, 
        const std::string& directory/* = ""*/ )
{
  TH1F* result = 0;
  if(directory.size()) result = dynamic_cast<TH1F*> (m_hist( name.c_str(), directory.c_str()));
  ...
  if(!result) {
    TH1F* histo = new TH1F(name.c_str(), title.c_str(), nbins, xbins);    
    Book(name, directory, dynamic_cast<TH1*> (histo));
    result = histo;
  }
...
//____________________________________________________________________________
TH1F* CycleBase::Book( const std::string& name, const std::string& title, int nbins, const double* xbins, 
        const std::string& directory/* = ""*/ )
{
  TH1F* result = 0;
  if(directory.size()) result = dynamic_cast<TH1F*> (m_hist( name.c_str(), directory.c_str()));
...
//____________________________________________________________________________
TH2F* CycleBase::Book( const std::string& name, const std::string& title, int nbinsx, float minx, float maxx, 
        int nbinsy, float miny, float maxy, const std::string& directory /*=""*/)
{

  TH2F* result = 0;
  if(directory.size()) result = dynamic_cast<TH2F*> (m_hist( name.c_str(), directory.c_str()));
  else result = dynamic_cast<TH2F*> (m_hist( name.c_str(), 0));

  if(!result) {
    TH2F* histo = new TH2F(name.c_str(), title.c_str(), nbinsx, minx, maxx, nbinsy, miny, maxy);    
    Book(name, directory, dynamic_cast<TH1*> (histo));
    result = histo;
  }
...
//     try
//     {
//         return dynamic_cast<TH2F*> (SCycleBase::Hist( name.c_str(), directory.c_str() ));
//     }
//     catch( const SError& error )
//     {
//         TH2F histo(name.c_str(), title.c_str(), nbinsx, minx, maxx, nbinsy, miny, maxy);
// 	if(directory.size()) return SCycleBase::Book( histo, directory.c_str() );
// 	else return SCycleBase::Book( histo, 0 );
//         throw;
//     }
...
//____________________________________________________________________________
TH2F* CycleBase::Book( const std::string& name, const std::string& title, int nbinsx, const double* xbins, 
        int nbinsy, float miny, float maxy, const std::string& directory /*=""*/)
{
  ...
  if(!result) {
    TH2F* histo = new TH2F(name.c_str(), title.c_str(), nbinsx, xbins, nbinsy, miny, maxy);    
    Book(name, directory, dynamic_cast<TH1*> (histo));
    result = histo;
  }
        ...
        int nbinsy, const double* ybins, const std::string& directory /*=""*/)
{
  ...
  if(!result) {
    TH2F* histo = new TH2F(name.c_str(), title.c_str(), nbinsx, minx, maxx, nbinsy, ybins);    
    Book(name, directory, dynamic_cast<TH1*> (histo));
    result = histo;
  }
...
//____________________________________________________________________________
TH2F* CycleBase::Book( const std::string& name, const std::string& title, int nbinsx, const float* xbins, 
        int nbinsy, const float* ybins, const std::string& directory /*=""*/)
{
  ...
  if(!result) {
    TH2F* histo = new TH2F(name.c_str(), title.c_str(), nbinsx, xbins, nbinsy, ybins);    
    Book(name, directory, dynamic_cast<TH1*> (histo));
    result = histo;
  }
...
void CycleBase::PublishHists()
{
  ...
  TH1F* h_list = 0;
  if(m_histoMap->size()) h_list = SCycleBase::Book( TH1F((std::string(GetName()) + "_hist_list").c_str(), "", 1, 0, 1) );
  else return;
...
void CycleBase::CollectHists()
{
  TH1F* h_list = 0;
  try {
    h_list = dynamic_cast<TH1F*> (SCycleBase::Hist( (std::string(GetName()) + "_hist_list").c_str() ));
...
//______________________________________________________________________________
bool CycleBase::CutFlow( const std::string& cut_name, bool requirement, bool skip_event/* = false*/, float weight/* = 1.0*/,const std::string& channel/* = ""*/) throw( SError )
{
    const int ncuts = 30;
    TH1F* h_cut_flow     = Book("h_cut_flow", "", ncuts, 0, ncuts);
    TH1F* h_raw_cut_flow = Book("h_raw_cut_flow", "", ncuts, 0, ncuts);
    ...
    TH1F* h_channel_cut_flow = 0;
    TH1F* h_channel_raw_cut_flow = 0;
      
    if (channel != ""){
      h_channel_cut_flow     = Book(Form("h_cut_flow_%s",channel.c_str()), "", ncuts, 0, ncuts);
      h_channel_raw_cut_flow = Book(Form("h_raw_cut_flow_%s",channel.c_str()), "", ncuts, 0, ncuts);

      h_channel_cut_flow->GetXaxis()->SetBinLabel(m_cut_index+1, cut_name.c_str());
      h_channel_raw_cut_flow->GetXaxis()->SetBinLabel(m_cut_index+1, cut_name.c_str());    
    }
    ...
    {         
        ...
        if (channel != ""){
           h_channel_cut_flow->Fill(m_cut_index, weight);
           h_channel_raw_cut_flow->Fill(m_cut_index);         
	}    
    }
    m_cut_index++;
...
void CycleBase::PrintCutFlow(const SInputData& id) throw( SError )
{
    ...
    for(int i=0; i != Hist("h_raw_cut_flow")->GetNbinsX(); ++i)
    {
	  ...
	  //<< (int) (weighted + 0.5) //round
           << weighted //round
           << "  " << std::setw(10) << std::right << std::scientific
           << (int) raw
...
{
    m_is_data = (id.GetType() == "data" || id.GetType() == "Data");
}

//____________________________________________________________________________
void CycleBase::m_set_mctype(const SInputData& id)
{
  const TString tp = id.GetType();
  if (tp == "MC11") m_mctype = AtlasSFrame::MC11;
  if (tp == "MC12a") m_mctype = AtlasSFrame::MC12a;
  if (tp == "MC12b") m_mctype = AtlasSFrame::MC12b;
  if (tp == "MC12c") m_mctype = AtlasSFrame::MC12c;
  m_logger << ALWAYS << "Setting AtlasSFrame::mctype to " << m_mctype << SLogger::endmsg;
}

//____________________________________________________________________________
void CycleBase::m_set_ds_version(const SInputData& id)
{
    m_ds_version = id.GetVersion().Data();
}
