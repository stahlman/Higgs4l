...
class CycleBase: public SCycleBase
{
  ...
  friend class ToolBase;
	...
	inline AtlasSFrame::mctype get_mctype() const { return m_mctype; }
	inline std::string sframe_dir() const { return c_sframe_dir; }
        inline const std::string& ds_version() const { return m_ds_version; }
        inline void Set_MC_channel_number(long number) {*m_MC_channel_number = number;}


	//helper storing configuration
        void InsertMetadata(const std::map<std::string, std::string> &config, const char* name);
	...
	TH1* Hist(const std::string& name, const std::string& directory = "" );
        ...
        // support for various TH1F and TH2F constructors
	TH1F* Book( const std::string& name, const std::string& title, int nbins, float min, float max, 
		    const std::string& directory = "" );
	TH1F* Book( const std::string& name, const std::string& title, int nbins, const float* xbins, 
		    const std::string& directory = "" );
	TH1F* Book( const std::string& name, const std::string& title, int nbins, const double* xbins, 
		    const std::string& directory = "" );
	TH2F* Book( const std::string& name, const std::string& title, int nbinsx, float minx, float maxx, 
		    int nbinsy, float miny, float maxy, const std::string& directory = "");
	TH2F* Book( const std::string& name, const std::string& title, int nbinsx, const double* xbins, 
		    int nbinsy, float miny, float maxy, const std::string& directory = "");
	TH2F* Book( const std::string& name, const std::string& title, int nbinsx, float minx, float maxx, 
		    int nbinsy, const double* ybins, const std::string& directory = "");
	TH2F* Book( const std::string& name, const std::string& title, int nbinsx, const float* xbins, 
		    int nbinsy, const float* ybins, const std::string& directory = "");
	TH2F* Book( const std::string& name, const std::string& title, int nbinsx, const double* xbins, 
		    int nbinsy, const double* ybins, const std::string& directory = "");


        bool Cut(const std::string& cut_name, bool requirement, bool skip_event = false) throw( SError );
        bool CutFlow( const std::string& cut_name, bool requirement, bool skip_event = false, float weight = 1.0, const std::string& channel = "") throw( SError );
        void ResetCutIndex();
	...
	void m_CollectToolHists();
        ...
        void m_set_mctype(const SInputData& id);
        void m_set_ds_version(const SInputData& id);
        ...
        bool m_is_data;
	AtlasSFrame::mctype m_mctype;
        std::string m_ds_version;
        SSummedVar<long> m_all_events;
        SSummedVar<long> m_passed_events;
        SSummedVar<long> m_MC_channel_number;
