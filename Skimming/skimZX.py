...
if FILETYPE != "mc":
    ...
    #for i in range(100): ###only for tests...
        ...
        for j_ele in range(n_ele):
            ...
            if not ( clus_pt>25000 
                     or evtChain.el_pt[j_ele]>=25000 
                     or evtChain.el_Et[j_ele]>=25000
                     or evtChain.el_cl_pt[j_ele]>=25000):
                continue
            #cut on Si hit
            if not nSihits>=1:
                continue
        ...
        for j_muon in range(n_muon_staco):
            ...
            if not ((evtChain.mu_staco_isCombinedMuon[j_muon]==1 or evtChain.mu_staco_isSegmentTaggedMuon[j_muon]==1) and (evtChain.mu_staco_author[j_muon]==6 or evtChain.mu_staco_author[j_muon]==7) and evtChain.mu_staco_pt[j_muon]>=25000):
                continue
        ...
        #print 'n_ele_good: '+str(n_ele_good)+', bestmasselediff: '+str(bestmasselediff)+'n_muon_good: '+str(n_muon_good)+', bestmassmuondiff: '+str(bestmassmuondiff)
        #if n_muon_good >= 2: print str(bestmassmuondiff)
        if ((n_ele_good>=2 and bestmasselediff < 20000) or (n_muon_good>=2 and bestmassmuondiff < 20000)):
