...
public:
  ...
  THStack * drawStackAdd(std::string hname, std::string hnameSave,std::string plotTitle, std::string xtitle, std::string ytitle="Entries", std::string opt = "", bool log=true, int Rebin = 1, double xmin = 999, double xmax = 999, double ymin=999, double ymax=999);
  ...
  TCanvas * newCanvas(std::string canvasTitle="NONE");
...
StackMgr::StackMgr(string outdir, string tag) : 
  m_debug(false), m_datafile(0), m_luminosity(1), m_nCanvas(0)
{
  if (m_debug) cout << "In StackMgr()" << endl;
  
  if (m_debug) cout << "Now altering global junk" << endl;
...
void StackMgr::addMCDataset(string filename, string dataname, int color,float scalefac,string dir){
  ...
  f = new TFile("output_masscut.root","RECREATE");
...
void StackMgr::addMCDataset_signal(string filename, string dataname, int color,float scalefac,string dir){
  ...
  f = new TFile("output_masscut.root","RECREATE");
  https://svnweb.cern.ch/trac/atlasgrp/browser/Physics/Exotic/Analysis/Dilepton/Resonance/Papers/Y2011/COM_PLHC/com_2011.pdf
  return;
}
...
THStack * StackMgr::drawStackAdd(string hname, string hnameSave, string plotTitle, string xtitle, string ytitle, string opt, bool log,int RebinFac, double xmin, double xmax, double ymin, double ymax){
 ...
 */  
 ...
 //sprintf(writetext4,plotTitle.c_str());
 ...
 sprintf(writetext1,"\\sqrt{s} = 8 TeV");
  ...
  if(opt == "signal" && Signal_Vec.size() > 0){
    ...
    t->DrawLatex(0.2,0.70,writetext1);
     ...
     leg_hist = new TLegend(0.60,leg_ymax-m_currentHistVec.size()*0.07,0.82,0.92);
     t->DrawLatex(0.2,0.91,writetext4);
     t->SetTextSize(0.04);     
     t->DrawLatex(0.2,0.84,writetext3);
     t->DrawLatex(0.2,0.76,writetext1);
     leg_integral = new TLegend(0.803,leg_ymax-m_currentHistVec.size()*0.07,0.87,0.92);
  }
  else{
  ...
  cout << "Integral of Data: " << integralData << " +/- " << integralDataError << " or " << sqrt(integralData) << std::endl;
  integralDataError = sqrt(integralData);
  
  leg_hist->AddEntry(Data,"Data 2012","P");
  ...
  leg_hist->AddEntry("total_mc","Total MC");
  ...
  h_ratio->Draw("E0"); //for axis,range      
  mc_stack_sum->Draw("SAME E2");
  //mc_stack_sum->Draw("E2");
  line->Draw("SAME");
  h_ratio->Draw("SAME E0");//so points lie on top of line       
...
TCanvas * StackMgr::newCanvas(string canvasTitle){
  ...
  m_canvas = new TCanvas(canvasTitle.c_str(),canvasTitle.c_str(),10,10,800,600+200); 
